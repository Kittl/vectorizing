...
def dilate(matrix: np.ndarray) -> np.ndarray:
    """Dilate labels with a two-by-two kernel and OpenCV's default anchor."""
    kernel = np.ones((2, 2))
...
def enhance(
    ...
    """Remove components mostly covered by neighboring colors, then fill holes."""
    ...
    # skimage's default connectivity includes diagonal neighbors.
