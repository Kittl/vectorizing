...
class CubicBezier:
    ...
    def is_flat_enough(self, tol = 1e-2):
    ...
    def flatten(self, scale = 1):
        if self.is_flat_enough():
            return [self.p0 * scale, self.p3 * scale]

        c1, c2 = self.subdivide()
        return c1.flatten() + c2.flatten()
    
    def to_clipper_geo(self, scale = 1):
        return self.flatten(scale)
    
    def move_to(self):
        return f'M {self.p0[0]},{self.p0[1]} '

    def draw(self):
        return f'C {self.p1[0]},{self.p1[1]} {self.p2[0]},{self.p2[1]} {self.p3[0]},{self.p3[1]} '